...
exclude_dir=/mnt,$HOME/Sync,$XDG_CACHE_HOME
...
if [ $is_mounted ]; then
  ...
  if [ -f "$target_path/$backup_name-latest.tar" ]; then
    ...
    cd $target_path && tar upvf $backup_name-latest.tar --exclude=/mnt --exclude=/home/clover/Sync --exclude=$XDG_CACHE_HOME /
  else
    echo -e "'$backup_name-latest.$compress_opt' doesn't exist, proceed to initial backup procedure"
    cd $target_path && tar cpvf $backup_name-latest.tar --exclude=/mnt --exclude=/home/clover/Sync --exclude=$XDG_CACHE_HOME /
  fi
else
